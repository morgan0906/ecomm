...
class Mail
{
	...
	$message = self::emailHeader().$message.self::emailFooter();
		...
		$email = new \PHPMailer();
		$email->From      = 'info@thirstybrush.co.uk';
		$email->FromName  = 'Thirsty Brush & Co';
	...
	$message = '<br><table width="95%" cellpadding="10" cellspacing="0" border="1" style="font-family:arial">';
	$message .= '<tr><td colspan="2" bgcolor="#EAEAEA"><strong>You have a website enquiry</strong></td></tr>';
	$message .= '<tr><td width="30%">Contact Name:</td><td>'.$_POST['name'].'</td></tr>';
	$message .= '<tr><td>Contact Number:</td><td>'.$_POST['phone'].'</td></tr>';
	$message .= '<tr><td>Email Address:</td><td>'.$_POST['email'].'</td></tr>';
	$message .= '<tr><td>Message:</td><td>'.$_POST['message'].'</td></tr>';
	$message .= '</table>';

	self::send('info@thirstybrush.co.uk', $message, 'Enquiry Form', COMPANY_NAME);
        ...
        "content" style="background-color: #FFFFFF">
          <tr>
            <td id="templateContainerHeader" valign="top" mc:edit="welcomeEdit-01">
              <p style="text-align:center;margin:0;padding:0;"><img width="200" src="'.DOMAIN.'/images/logo.jpg" style="display:inline-block;"></p>
            </td>
          </tr>
