...
#<!--------------Retyped versionof countItems----------------->
  public function countItems123()
  {
    $total = 0;
    foreach($this->getAll() as $row){
...
#<!--------------Retyped versionof countItems----------------->
	...
	if($promoObj->getPromoCodeId()){

	$promo_code = $promoObj->find($promoObj->getPromoCodeId());
		...
		if( $promo_code->percentage ){
			...
			if( $this->subTotal() && Tools::showDiscountPricePercentage($this->subTotal(), $promo_code->percentage) >= 30 ){
	...
	if( $this->subTotal() >= 30 && !$promoObj->getPromoCodeId() ){
	...
	if( $amount == '' ){
		...
		$_SESSION[SESSION.'shipping'] = '2.99';
