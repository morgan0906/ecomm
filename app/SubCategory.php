...
class SubCategory extends ObjectModel
{
    ...
    protected $fillable = ['title', 'seo_url', 'category_id', 'meta_description'];
    protected $rules = ['title' => 'required', 'seo_url' => 'required|unique:sub_categories', 'category_id' => 'required'];
	...
	return $this->execute('SELECT *, sub_categories.title AS sub_category_title, sub_categories.seo_url AS sub_category_seo_url, sub_categories.id AS sub_category_id  
					...
					WHERE sub_categories.deleted_at IS NULL ORDER BY categories.id ASC ', []);
    ...
    public function getProductForImage($sub_category_id)
    {

	return $this->execute('SELECT * FROM products WHERE sub_category_id LIKE ? AND deleted_at IS NULL ORDER BY id DESC LIMIT 1 ', [ "%\"".$sub_category_id."\"%" ]);
